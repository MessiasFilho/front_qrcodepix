  ...
  <div class="w-full min-h-screen flex flex-col">
    ...
    <div class="bg-gray-100 rounded-md">
      <div class="flex items-center bg-white rounded-t-md p-2 h-full shadow-2xl">
        <input
          v-model="search"
          class="w-full outline-none border p-1 rounded-md"
          type="text"
      ...
      <div class="flex items-center gap-2 bg-white shadow-2xl rounded-b-md p-2">
        ...
        >
          <div class="inline-flex gap-2 pr-6 bg-white">
            <button
              v-for="(cat, i) in categoryStore.categoryes"
              :key="i"
    ...
    <div class="">
      <MotionComponent animation="fade-up">
        <main class="w-full h-full flex flex-col rounded-md">
          <div
            class="w-full h-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-1"
          >
            <div
            ...
            >
              <div @click="navigatePage(item.id)">
                <div
                  class="w-full aspect-[4/3] overflow-auto shadow-xl rounded-t-md bg-gray-100"
                >
                  <img
                    ...
                    loading="lazy"
                    decoding="async"
                    class="w-full h-full object-cover rounded-t-md shadow-2xl border"
                  />
                </div>
                <div class="bg-white p-2 rounded-b-md shadow-xl">
                  ...
                  <div class="mt-2 flex items-center justify-between">
                    <div
                      class="flex space-x-2 items-center text-[var(--primary)] font-bold"
                    >
...
const filterProducts = computed(() => {
  return productsStore.products.filter((prod) => {
    const matchesCategory =
      !selectCategory.value || prod.category_name === selectCategory.value;
    const matchesSearch =
      !search.value ||
...
const navigatePage = (id: number) => {
  ...
  navigateTo(`${base}/${id}`.replace(/\/{2,}/g, "/"));
};
